...
public class MitigationColorPickerWindow : Window, IDisposable
{
    private Configuration Configuration;
    private RaidBuffTrackerPlugin raidBuffTrackerPlugin;
    private ImmutableSortedSet<UIColor> colors;

    public MitigationColorPickerWindow(RaidBuffTrackerPlugin raidBuffTrackerPlugin, ExcelSheet<UIColor>? uiColorExcel) : base(
        "Mitigation Color Picker", ImGuiWindowFlags.NoScrollbar | ImGuiWindowFlags.NoResize |
    ...
    public override void Draw()
    {
        ...
        if (ImGui.Button("Test Color"))
        {
            ...
            builder.AddUiForeground((ushort)Configuration.MitigationColor);
        ...
        if (raidBuffTrackerPlugin.UiColors != null)
        {
            for (var index = 0; index < colors.Count; index++)
            {
                ...
                if (ImGui.ColorButton(z.RowId.ToString(), new Vector4(
                                          ...
                                          (float)temp[0] / 255)))
                {
                    Configuration.MitigationColor = z.RowId;
                    Configuration.Save();
                }
            }
        }
